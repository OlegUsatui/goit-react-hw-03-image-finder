      ...
      getImages(searchImage, page).then(({ hits }) => {
        if (hits.length === 0) {
          ...
          if (prevState && prevState.searchImage === this.state.searchImage) {
            ...
            this.setState({ gallery: hits, reqStatus: "resolved" });
          }
        }
      });
    }
  ...
  render() {
    return (
      <div className={css.App}>
        ...
        {this.state.reqStatus === "resolved" && (
          <Button text="Load more" onLoadMoreClick={this.onLoadMoreClick} />
        )}
        ...
        {this.state.largeImage && (
          <Modal closeModal={this.closeModal} image={this.state.largeImage} />
        )}
      </div>
    );
  }
